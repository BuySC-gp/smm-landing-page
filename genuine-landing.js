...
// =============================================================================
// NAVBAR TOP GP - SOLUTION 100% SAFE (JAVASCRIPT UNIQUEMENT)
// =============================================================================
(function() {
    'use strict';
    
    function styleNavbarTop() {
        // Trouver la navbar TOP (celle avec logo à gauche)
        const navbars = document.querySelectorAll('nav.navbar');
        let topNavbar = null;
        
        for (let nav of navbars) {
            // Navbar TOP = celle qui contient le logo ET balance/account/logout
            if (nav.querySelector('.component-navbar-logo img') && 
                nav.querySelector('a[href*="balance"], a[href*="funds"], a[href*="account"], a[href*="logout"]')) {
                // ET qui n'est PAS dans sidebar
                if (!nav.closest('.component_private_sidebar')) {
                    topNavbar = nav;
                    break;
                }
            }
        ...
        if (!topNavbar) {
            console.log('❌ Navbar top not found');
            return;
        }
        
        console.log('✅ Navbar top found:', topNavbar);
        ...
        // === LOGO ===
        const logo = topNavbar.querySelector('.component-navbar-logo img, .sidebar-block__top-logo');
        if (logo) {
            logo.style.cssText = `
            ...
            `;
            logo.parentElement.style.alignItems = 'center';
        }
        
        // === MENU ITEMS ===
        const navItems = topNavbar.querySelectorAll('.navbar-nav > .nav-item .nav-link');
        console.log('📋 Menu items:', navItems.length);
        ...
        navItems.forEach((link, index) => {
            ...
            // === BALANCE (contient $ ou balance/funds) ===
            if (text.includes('$') || href.includes('balance') || href.includes('funds')) {
                link.style.cssText += `
                    background: linear-gradient(135deg, #E8F0FF, #F0F4FF) !important;
                ...
                // Hover
                link.onmouseover = () => {
                    link.style.background = 'linear-gradient(135deg, #D0E0FF, #E8F0FF)';
                    link.style.transform = 'translateY(-1px)';
                    link.style.boxShadow = '0 4px 12px rgba(0,71,171,0.15)';
                };
                link.onmouseout = () => {
                    link.style.background = 'linear-gradient(135deg, #E8F0FF, #F0F4FF)';
                    link.style.transform = 'translateY(0)';
                    link.style.boxShadow = 'none';
                };
            }
            // === ACCOUNT ===
            else if (href.includes('account') || text.toLowerCase().includes('account')) {
                link.style.cssText += `
            ...
            // === LOGOUT ===
            else if (href.includes('logout') || text.toLowerCase().includes('logout')) {
                link.style.cssText += `
                ...
                link.onmouseover = () => {
                    link.style.background = 'linear-gradient(135deg, #FEE2E2, #FECACA)';
                    link.style.transform = 'translateY(-1px)';
                    link.style.boxShadow = '0 4px 12px rgba(220,38,38,0.15)';
                };
                link.onmouseout = () => {
                    link.style.background = 'linear-gradient(135deg, #FEF2F2, #FEE2E2)';
                    link.style.transform = 'translateY(0)';
                    link.style.boxShadow = 'none';
                };
            }
        ...
        // Gap entre items
        const navContainer = topNavbar.querySelector('.navbar-nav');
        if (navContainer) {
        ...
        console.log('✅ Navbar top styled perfectly');
    }
    
    // Lancer après chargement + observer mutations
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', styleNavbarTop);
    } else {
        styleNavbarTop();
    }
    
    // Relancer si DOM change (MTP dynamique)
    const observer = new MutationObserver(styleNavbarTop);
