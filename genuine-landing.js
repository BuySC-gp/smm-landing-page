...
// =============================================================================
// MODULE: STATS BAR PREMIUM REDESIGN (PAGE NEW ORDER)
// =============================================================================
(function () {
  'use strict';
  ...
  console.log('🎯 [STATS BAR] Page detected - Loading Premium Redesign...');

  function initStatsBarRedesign() {
    // Trouver le bloc de stats natif
    const nativeStatsBlock = document.querySelector('#block_206') ||
      document.querySelector('.totals')?.closest('[id^="block_"]') ||
      document.querySelector('.totals')?.parentElement;

    if (!nativeStatsBlock || document.getElementById('gp-stats-bar-premium')) {
      return;
    }

    console.log('✅ [STATS BAR] Native block found:', nativeStatsBlock);

    // Extraire les données des stats natives
    const statsData = extractStatsData(nativeStatsBlock);
    console.log('📊 [STATS BAR] Data extracted:', statsData);

    // Créer la nouvelle barre premium
    const premiumBar = document.createElement('div');
    premiumBar.id = 'gp-stats-bar-premium';
    premiumBar.innerHTML = createPremiumStatsHTML(statsData);

    // Injecter les styles
    injectPremiumStyles();

    // Remplacer le bloc natif
    nativeStatsBlock.style.display = 'none';
    nativeStatsBlock.parentElement.insertBefore(premiumBar, nativeStatsBlock);

    console.log('✅ [STATS BAR] Premium Redesign Applied!');
  }

  function extractStatsData(block) {
    const data = {
      username: 'User',
      spentBalance: '$0.00',
      accountBalance: '$0.00',
    ...
    // Chercher les éléments de stats
    const rows = block.querySelectorAll('.row > div, .totals > div > div, [class*="col"]');

    rows.forEach(row => {
      const text = row.textContent?.toLowerCase() || '';
      const valueEl = row.querySelector('span, strong, b, h3, h4, .value') || row;
      const value = valueEl?.textContent?.trim() || '';

      if (text.includes('spent') || text.includes('dépensé')) {
        data.spentBalance = value.match(/\$[\d.,]+/)?.[0] || value;
      } else if (text.includes('account') && text.includes('balance') || text.includes('solde')) {
        data.accountBalance = value.match(/\$[\d.,]+/)?.[0] || value;
      } else if (text.includes('active') || text.includes('services') || text.includes('actif')) {
        data.activeServices = value.match(/\d+/)?.[0] || value;
      } else if (text.includes('test-account') || text.includes('user') || text.includes('account-')) {
        data.username = value.split('\n')[0]?.trim() || value;
      }
    });

    // Fallback: essayer de récupérer depuis les éléments directs
    const allTexts = block.querySelectorAll('*');
    allTexts.forEach(el => {
      const t = el.textContent?.trim();
      if (t?.startsWith('$') && t?.length < 15) {
        if (!data.spentBalance.includes('$') || data.spentBalance === '$0.00') {
          data.spentBalance = t;
        } else if (!data.accountBalance.includes('$') || data.accountBalance === '$0.00') {
          data.accountBalance = t;
        }
      }
    });
  ...
  function createPremiumStatsHTML(data) {
    return `
      <div class="gp-stats-container">
        <!-- Welcome Card -->
        <div class="gp-stat-card gp-stat-welcome">
          <div class="gp-stat-icon" style="background: linear-gradient(135deg, #6366f1, #8b5cf6);">
          ...
          <div class="gp-stat-content">
            <span class="gp-stat-value gp-stat-username">${data.username}</span>
            <span class="gp-stat-label">Welcome Back!</span>
          </div>
        </div>

        <!-- Spent Balance -->
        <div class="gp-stat-card">
          <div class="gp-stat-icon" style="background: linear-gradient(135deg, #f59e0b, #fbbf24);">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <line x1="12" y1="1" x2="12" y2="23"/>
              <path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>
            </svg>
          ...
          <div class="gp-stat-content">
            <span class="gp-stat-value">${data.spentBalance}</span>
            <span class="gp-stat-label">Spent Balance</span>
          </div>
        </div>

        <!-- Account Balance -->
        <div class="gp-stat-card gp-stat-highlight">
          <div class="gp-stat-icon" style="background: linear-gradient(135deg, #10b981, #34d399);">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <rect x="1" y="4" width="22" height="16" rx="2" ry="2"/>
              <line x1="1" y1="10" x2="23" y2="10"/>
            </svg>
          ...
          <div class="gp-stat-content">
            <span class="gp-stat-value gp-stat-balance">${data.accountBalance}</span>
            <span class="gp-stat-label">Account Balance</span>
          </div>
        </div>

        <!-- Active Services -->
        <div class="gp-stat-card">
          <div class="gp-stat-icon" style="background: linear-gradient(135deg, #3b82f6, #60a5fa);">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/>
            </svg>
          </div>
          <div class="gp-stat-content">
            <span class="gp-stat-value">${data.activeServices}</span>
  ...
  function injectPremiumStyles() {
    if (document.getElementById('gp-stats-premium-styles')) return;

    const styles = document.createElement('style');
    styles.id = 'gp-stats-premium-styles';
    styles.textContent = `
      /* === STATS BAR PREMIUM STYLES === */
      #gp-stats-bar-premium {
        width: 100%;
        margin-bottom: 24px;
      }
  ...
  // Initialisation
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => setTimeout(initStatsBarRedesign, 300));
  } else {
    setTimeout(initStatsBarRedesign, 300);
  }
